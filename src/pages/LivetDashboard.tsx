...
export default function LivetDashboard() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // TODO: Criar webhook específico da Livet no N8N apontando para as tabelas da Livet
      // Trocar a URL abaixo pelo webhook dedicado: /webhook/busca-resumos-livet
      const res = await fetch('https://n8n.trafficsolutions.cloud/webhook/busca-resumos-livet');
      if (res.ok) {
        ...
        // Filtrar apenas vendedores da Livet pelo ID
        if (parsed) {
          parsed = {
            vendedores: parsed.vendedores.filter(v => LIVET_IDS.includes(v.vendedor_id)),
          };
        }
      ...
      console.error('Erro ao buscar dados Livet:', err);
    }
    setLoading(false);
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white p-6 md:p-8">
        ...
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-white">
              Painel LIVET — Resumos & Conversas
            </h1>
          </div>
          <p className="text-gray-400 text-sm mt-1">
            Análise completa de conversas, qualidade de leads e vendedores · iPhone 12 · Samsung · iPhone 6
          </p>
        </div>
        <Button
          onClick={fetchData}
      ...
      <div className="flex flex-wrap items-center gap-2 mb-6 p-3 bg-white/[0.03] border border-blue-500/20 rounded-xl">
        <span className="text-xs text-gray-500 mr-1">Dispositivo:</span>
