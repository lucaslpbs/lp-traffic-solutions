...
// ── Instâncias da NCSaude ─────────────────────────────────────────────────────
// IDs dos vendedores da NCSaude conforme tabela de vendedores (instâncias 8, 9, 10, 15)
const NCSAUDE_IDS = [8, 9, 10, 15];
...
export default function NCSaudeDashboard() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // TODO: Criar webhook específico da NCSaude no N8N apontando para as tabelas da NCSaude
      // Trocar a URL abaixo pelo webhook dedicado: /webhook/busca-resumos-ncsaude
      const res = await fetch('https://n8n.trafficsolutions.cloud/webhook/busca-resumos-ncsaude');
      if (res.ok) {
        ...
        // Filtrar apenas vendedores da NCSaude pelo ID
        if (parsed) {
          parsed = {
            vendedores: parsed.vendedores.filter(v => NCSAUDE_IDS.includes(v.vendedor_id)),
          };
        }
      ...
      console.error('Erro ao buscar dados NCSaude:', err);
    }
    setLoading(false);
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white p-6 md:p-8">
        ...
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-white">
              Painel NCSaude — Resumos & Conversas
            </h1>
          </div>
          <p className="text-gray-400 text-sm mt-1">
            Análise completa de conversas · Raquel · Arthur · Camila · Número Único
          </p>
        </div>
        <Button
          onClick={fetchData}
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        {[
          { title: 'Total Resumos', value: total, icon: Users, color: '#10b981' },
          ...
          { title: 'Qual. Vendedor (Média)', value: avgQualidadeVendedor, icon: UserCheck, color: '#10b981' },
        ].map(kpi => (
          <div
            key={kpi.title}
