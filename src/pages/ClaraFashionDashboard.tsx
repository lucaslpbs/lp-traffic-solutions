...
// ── Instância da Clara Fashion ────────────────────────────────────────────────
// ID do vendedor da useclarafashion conforme tabela de vendedores (instância 11)
const CLARA_IDS = [11];
...
export default function ClaraFashionDashboard() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      // TODO: Criar webhook específico da Clara Fashion no N8N apontando para as tabelas dela
      // Trocar a URL abaixo pelo webhook dedicado: /webhook/busca-resumos-clarafashion
      const res = await fetch('https://n8n.trafficsolutions.cloud/webhook/busca-resumos-clarafashion');
      if (res.ok) {
        ...
        // Filtrar apenas vendedor da Clara Fashion pelo ID
        if (parsed) {
          parsed = {
            vendedores: parsed.vendedores.filter(v => CLARA_IDS.includes(v.vendedor_id)),
          };
        }
      ...
      console.error('Erro ao buscar dados Clara Fashion:', err);
    }
    setLoading(false);
  };
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-white p-6 md:p-8">
        ...
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            <h1 className="text-2xl md:text-3xl font-bold text-white">
              Painel Clara Fashion — Resumos & Conversas
            </h1>
          </div>
      ...
      {/* Navegação por vendedor */}
      <div className="flex flex-wrap items-center gap-2 mb-6 p-3 bg-white/[0.03] border border-pink-500/20 rounded-xl">
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
        {[
          { title: 'Total Resumos', value: total, icon: Users, color: '#ec4899' },
          ...
          { title: 'Qual. Vendedor (Média)', value: avgQualidadeVendedor, icon: UserCheck, color: '#ec4899' },
        ].map(kpi => (
          <div
            key={kpi.title}
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                              <div>
                                ...
                                {r.proximo_passo_sugerido && (
                                  <div className="mt-4">
                                    <h4 className="text-xs font-semibold text-pink-400 uppercase mb-1 flex items-center gap-1.5">
