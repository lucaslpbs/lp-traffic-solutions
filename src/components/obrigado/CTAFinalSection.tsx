import { WHATSAPP_URL } from './constants';

export default function CTAFinalSection() {
  return (
    <section
      className="py-20 md:py-28 relative overflow-hidden"
      style={{ background: 'var(--gradient-hero)' }}
    >
      {/* Blobs decorativos */}
      <div
        className="absolute -top-32 -right-32 w-96 h-96 rounded-full blur-3xl opacity-20 animate-pulse"
        style={{ background: 'hsl(217 91% 60%)' }}
        aria-hidden="true"
      />
      <div
        className="absolute -bottom-32 -left-32 w-96 h-96 rounded-full blur-3xl opacity-15 animate-pulse"
        style={{ background: 'hsl(217 91% 45%)', animationDelay: '1s' }}
        aria-hidden="true"
      />

      <div className="relative z-10 container mx-auto px-6 max-w-3xl text-center">

        {/* Badge de urgência */}
        <div
          className="inline-flex items-center gap-2 px-4 py-2 rounded-full text-sm font-semibold mb-8"
          style={{
            background: 'hsl(0 84% 60% / 0.15)',
            border: '1px solid hsl(0 84% 60% / 0.4)',
            color: 'hsl(0 84% 75%)',
          }}
        >
          <span className="w-2 h-2 rounded-full bg-red-400 animate-ping inline-block" />
          Vagas limitadas para novos clientes neste mês
        </div>

        {/* Headline */}
        <h2 className="text-3xl md:text-5xl font-black text-white leading-tight mb-6">
          Não espere o concorrente{' '}
          <span
            className="bg-clip-text text-transparent"
            style={{ backgroundImage: 'var(--gradient-modern)' }}
          >
            sair na frente
          </span>
        </h2>

        <p className="text-white/70 text-lg md:text-xl mb-10 max-w-xl mx-auto leading-relaxed">
          Nossa equipe já está analisando o seu perfil. Se você não quiser esperar o nosso
          retorno, clique abaixo e fale diretamente com um especialista agora.
        </p>

        {/* CTA pulsante */}
        <a
          href={WHATSAPP_URL}
          target="_blank"
          rel="noopener noreferrer"
          className="ob-pulse-green inline-flex items-center gap-3 px-10 py-5 rounded-xl text-white font-bold text-xl transition-all duration-300 hover:scale-105"
          style={{ background: 'hsl(142 76% 36%)' }}
        >
          <svg viewBox="0 0 24 24" className="w-7 h-7 fill-white flex-shrink-0">
            <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
          </svg>
          Quero falar agora no WhatsApp
        </a>

        {/* Horário de atendimento */}
        <p className="text-white/40 text-sm mt-6 flex items-center justify-center gap-2">
          <svg viewBox="0 0 24 24" className="w-4 h-4 fill-none stroke-current" strokeWidth="2" strokeLinecap="round">
            <circle cx="12" cy="12" r="10" />
            <polyline points="12 6 12 12 16 14" />
          </svg>
          {/* TODO: Ajustar horário de atendimento */}
          Atendimento: segunda a sexta, das 9h às 18h (Horário de Brasília)
        </p>
      </div>
    </section>
  );
}
