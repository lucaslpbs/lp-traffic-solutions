...
export const ComissaoAdminPanel = ({
  ...
  return (
    <div className="rounded-xl border border-foreground/10 bg-card overflow-hidden">
      ...
      <div className="px-4 py-4 border-b border-foreground/5 bg-foreground/[0.03] grid gap-3 sm:grid-cols-[1fr_1fr_140px_auto] sm:items-end">
        <div className="space-y-1.5">
          <span className="text-xs text-muted-foreground font-medium">Indicador (cliente)</span>
        ...
        <div className="space-y-1.5">
          <span className="text-xs text-muted-foreground font-medium">Cliente que fechou</span>
        ...
        <div className="space-y-1.5">
          <span className="text-xs text-muted-foreground font-medium">Ticket mensal</span>
      ...
      <div className="border-t border-foreground/5">
        ...
        {loadingResgates ? (
          ...
          <p className="px-4 pb-4 text-sm text-muted-foreground">Nenhum resgate aguardando pagamento.</p>
        ) : (
              ...
              <div key={r.id} className="flex items-center gap-4 px-4 py-3">
                <div className="min-w-0 flex-1">
                  <p className="font-medium text-foreground truncate">
                    ...
                    <span className="ml-2 text-sm font-normal text-success">{formatBRL(r.valor_reais)}</span>
                  </p>
      ...
      <div className="border-t border-foreground/5">
        ...
        {loadingFechamentos ? (
          ...
          <p className="px-4 pb-4 text-sm text-muted-foreground">Nenhum fechamento lançado ainda.</p>
        ) : (
            ...
            {fechamentos.map((f) => (
              <div key={f.id} className="flex items-center gap-4 px-4 py-3">
                ...
                <span
                  className={
                    f.status === 'disponivel'
                      ? 'rounded-full border px-2 py-0.5 text-[11px] font-medium whitespace-nowrap bg-success/15 text-success border-success/30'
                      : 'rounded-full border px-2 py-0.5 text-[11px] font-medium whitespace-nowrap bg-foreground/10 text-muted-foreground border-foreground/10'
                  }
                >
