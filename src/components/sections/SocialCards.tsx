...
export default function SocialCards({ cards }: SocialCardsProps) {
  ...
  // Com MAX_VISIBLE cards todos ficam visíveis, mas a rotação continua
  // valendo a pena: é ela que permite trazer qualquer card para o centro.
  ...
  // Leva um card específico para o centro (clique no card ou no indicador).
  const goTo = useCallback((index: number) => {
    if (isAnimating.current || !needsPagination || index === centerIndex) return;
    isAnimating.current = true;
    const forwardSteps = ((index - centerIndex) % totalCards + totalCards) % totalCards;
    directionRef.current = forwardSteps * 2 <= totalCards ? "right" : "left";
        ...
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem]">
          {cards.map((card, index) => {
            ...
            return card.linkUrl ? (
              ...
              <div
                key={index}
                className={`fan-card ${needsPagination ? "cursor-pointer" : ""}`}
                onClick={() => goTo(index)}
              >
                {image}
              </div>
          ...
          <div className="flex items-center gap-2">
            {cards.map((_, i) => (
              <button
                ...
                className={`w-2 h-2 rounded-full transition-all duration-300 cursor-pointer ${i === centerIndex ? "bg-black/70 dark:bg-white/80 scale-[1.3]" : "bg-black/15 dark:bg-white/15 hover:bg-black/35 dark:hover:bg-white/35"}`}
              />
            ))}
          </div>
